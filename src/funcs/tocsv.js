...
export default class ConvertDbToCsv{
  ...
  createRow(empresa){
    var o = [];
    var fields = empresa.fields ? Object.values(empresa.fields) : [];
    ...
    /* Representantes */
    var representantes = this.getByGroup('representante', fields);
    ...
    /* Banderas rojas */
    var bs = this.searchFieldValueBySlug('banderas-rojas', fields);
    if(bs.length){
      bs = bs[0].bs;
      var bs_a = ['InscritoRUPC', 'NoInscritoRUPC', 'RegCompraNet'];
      var s = {
        47: [],
        48: []
      }
  ...
  searchFieldValue(field, fields){
    var r = fields.filter((f) => f.name == field);
    return r.length ? r[0].value : "";
  }

  searchFieldValueBySlug(field, fields){
    var r = fields.filter((f) => f.slug == field);
    return r.length ? r : [];
  }
  ...
  getTransferenciaGroup(group, fields){
    var o = {};
    var keys = [];
    var r = fields.filter((f) => f.group == "transferencia");
    ...
    filtered.map(function(_f){
      _f.map(function(field){
  ...
  getByGroup(group, fields, vk){
    ...
    var o = {};
    var keys = [];
    var r = fields.filter((f) => f.group == group);
    r.map(function(_f){
      if(!o[_f.guid]){
        o[_f.guid] = [];
      }
